...
| `config/` | 环境变量 + 4 份 YAML 策略配置 | 阈值/规则/评分/场景全部可配，改策略不改代码；启动期强校验，配置写错直接启动失败 |
...
| `ai/` | AI 归因 | 证据包只放事实；强 schema 校验 + 幻觉引用清理 + 漏答补齐；三级降级 |
...
| `ai_analysis` | 证据包 → LLM → 校验 → 降级 | **必然**产出结论（三级降级链） |
...
### ADR-3 三级降级，AI 不可用也必须出报告

**决策**：大模型 → 规则决策树 → 纯统计。降级路径与原因写入 `AIMeta.degraded`。
...
| 维度 | 实现 |
| --- | --- |
| 结构化日志 | structlog，`run_id` 通过 contextvar 贯穿，JSON 格式可直接采集到 ELK/Loki |
...
| 巡检串行执行 | 单次巡检 1~2 秒，暂不需要并行 | 规则可按维度并行，`EvalContext` 的缓存是线程安全的读多写少结构 |
