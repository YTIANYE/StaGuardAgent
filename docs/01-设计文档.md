...
sequenceDiagram
    ...
    D->>D: build_granularity_stats(服务/集群维度)
    D-->>O: 异常清单 + 根因簇 + 多粒度统计
    O->>O: 规则算分（可解释）
    O->>A: analyze(簇, 证据包)
    A->>A: 调用 LLM → 校验 → 必要时降级
...
| `collector/` | 数据接入抽象 | 两条通道共用同一份底层数据，结果必然一致；源可插拔，接 Prometheus 只加一个实现。`dataset_id` 是「回放哪份切片」的标识而非必需参数：内置源按它定位切片，实时源可忽略它、直接按窗口查 |
...
| `detect/` | 聚合、聚类与统计 | 去重、跨规则抑制、影响面回填、拓扑传播聚类、主根因加权选取；多粒度统计（服务 / 集群维度，只读聚合） |
| `ai/` | AI 归因 | 证据包只放事实；强 schema 校验 + 幻觉引用清理 + 漏答补齐；两级降级兜底 |
| `scoring.py` | 稳定性评分 | 错误预算口径 + 可解释扣分明细 + 最严格封顶生效 |
| `report/` | 报告输出 | 固定八章结构；控制台与 Markdown 信息密度取法不同 |
...
| `aggregate` | 去重、抑制、影响面回填、拓扑聚类、多粒度统计 | 异常清单为空但报告可出 |
...
| 维度 | 实现 |
| --- | --- |
| 结构化日志 | structlog，`run_id` 通过 contextvar 贯穿巡检全过程（CLI / HTTP / 调度器三条触发路径都在编排器入口绑定）；同时输出到标准错误与 `logs/staguard.log`（文件固定 JSON，10MB × 5 轮转），可直接采集到 ELK/Loki |
| 阶段耗时 | 每个阶段记录 `duration_ms`，报告附录里原样展示 |
| 运行指标 | `/metrics` 暴露巡检次数、异常数、触发结果分布，以及正在执行的巡检请求数 `staguard_pending_inspections`（HPA 扩容依据）（Prometheus 文本格式） |
...
| 模拟环境的「当前水位」 | 没有持续写入的数据源，用一个固定切片代表当前水位（`STAGUARD_LIVE_DATASET`，默认 S0 正常态） | 接入实时数据源即可，数据源按窗口查数据、忽略切片 ID，编排逻辑零改动 |
