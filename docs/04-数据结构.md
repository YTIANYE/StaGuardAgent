...
class InspectionReport:      # 报告渲染模块的唯一输入
    ...
    service_stats / cluster_stats        # 多粒度统计（见下）
```

### ServiceStat / ClusterStat（多粒度统计）

```python
class ServiceStat:                    # 服务维度：一个服务一行，含无异常的服务
    service / cluster / cluster_label / tier / owner / criticality
    anomaly_count: int                # 未抑制异常条数
    max_level: Severity | None        # 无异常为 null
    affected_instances: list[str]     # 只含拓扑里声明过的实例名
    total_instances: int
    rule_ids / worst_metric / worst_summary
    root_cause_of: list[str]          # 该服务作为主根因的簇 ID
    in_clusters: list[str]            # 该服务作为成员出现的簇 ID
    # affected_ratio / is_healthy

class ClusterStat:                    # 集群维度：一个集群一行
    cluster / label
    service_count / affected_service_count / anomaly_count / max_level
    affected_instances / total_instances / root_cause_count
    # affected_ratio / affected_service_ratio / is_healthy
```

两张表由 `staguard/detect/statistics.py` 一次算出，保证口径一致；
统计只读，不参与评分与分级。
...
- **`metric_points` 必须有查询计划统计信息**。两百多万行的大表如果没有 `sqlite_stat1`，
  SQLite 会按「表很小、分布均匀」的默认猜测给「按序列取 14 天历史」的查询选一个
  全数据集扫描的计划——实测单次巡检从 5 秒变成 46 秒、`make test` 从 2 分半变成 15 分半，
  而**结果完全正确、日志里没有任何异常**，只能靠耗时发现。
  `Database.optimize()` 负责在批量写入之后收集统计信息；
- `runs.dataset_id` 记录本次实际读的数据切片：场景巡检即场景编号，
  不带场景的巡检即 `live_dataset`。「当前水位巡检正常」这句话，读者有权知道看的是哪份数据；
...
| 文件 | 顶层结构 | 作用 |
| --- | --- | --- |
| `config/services.yaml` | `default_slo` + `clusters{}` + `services{}` | 服务清单、依赖拓扑、关键度、SLO、集群分组 |
...
运行期配置走环境变量（前缀 `STAGUARD_`，见 `.env.example`），其中与数据来源相关的两项：

| 变量 | 默认 | 作用 |
| --- | --- | --- |
| `STAGUARD_DEFAULT_SOURCE` | `file` | 默认数据通道：本地文件 / 模拟 HTTP 接口 |
| `STAGUARD_LIVE_DATASET` | `S0` | **不带场景巡检时，用哪份切片代表「当前水位」** |

第二个变量只对内置的文件源 / 模拟接口有意义：`scenario_id` 的职责是回放历史故障切片，
而不带场景的巡检（定时巡检、接口手动触发）问的是「现在什么水位」。
真实监控数据源按窗口查实时数据，可以直接忽略这个切片 ID——
这也是 `CollectRequest.dataset_id` 在契约上被定义为「数据切片 ID」而不是必需参数的原因。
