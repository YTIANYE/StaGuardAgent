...
**对比对象**：`run-20260925-100000-S0`（2026-09-26T17:35+08:00）
...
| 标准化清洗 | 成功 | 2591ms | 216000 个有效点 / 150 条时序；缺失率 0.00%，置信度 高 |
| 结构化存储 | 成功 | 193ms | 数据集 S1 就绪（216000 个指标点） |
| 动态基线 | 成功 | 1550ms | 计算 150 条基线，动态基线覆盖率 100%（归档 603894 点） |
| 规则巡检 | 成功 | 252ms | 16 条规则命中 59 项原始发现（252ms） |
| 聚合去重 | 成功 | 5ms | 59 条异常（抑制 9 条）聚成 1 个根因簇；多粒度统计 8 个服务 / 5 个集群 |
| 稳定性评分 | 成功 | 0ms | 规则算分 20.4（E 严重） |
| AI 智能分析 | 降级 | 4ms | 降级运行（配置中已关闭 AI 分析（STAGUARD_LLM_ENABLED=false））；1 条根因结论，toke… |
| 报告输出 | 成功 | 10ms | Markdown 报告 13232 字符，已写入 run-20260925-100000-S1.md |
