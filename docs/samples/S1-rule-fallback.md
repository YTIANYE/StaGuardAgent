...
本次巡检共识别 1 个根因簇，最严重的是 bank-channel 的连接池使用率 异常（P1 紧急），已扩散至 4 个服务。归因类别集中于：依赖故障。
...
- **AI 根因**：**依赖故障** — bank-channel 连接池被慢请求占满且延时同步恶化，指向其下游依赖响应退化而非自身容量不足；证据：连接池使用率 99.00%，接近打满，新请求将被拒绝（置信度 65%）
...
- **CLUS-01 · 依赖故障**
  - 检查 bank-channel 下游依赖的健康度与连接池占用，必要时熔断或降级

### 短期加固（本周内）

- [CLUS-01] 检查下游依赖健康度与连接池占用，必要时降级、熔断或切换备用链路。
...
**对比对象**：`run-20260925-100000-S0`（2026-09-26T02:57+08:00）
...
| 阶段 | 状态 | 耗时 | 说明 |
| --- | --- | ---: | --- |
| 数据采集 | 成功 | 674ms | file 通道采集 215676 个点 / 150 条时序（670ms） |
| 标准化清洗 | 成功 | 2651ms | 216000 个有效点 / 150 条时序；缺失率 0.00%，置信度 高 |
| 结构化存储 | 成功 | 199ms | 数据集 S1 就绪（216000 个指标点） |
| 动态基线 | 成功 | 1538ms | 计算 150 条基线，动态基线覆盖率 100%（归档 603894 点） |
| 规则巡检 | 成功 | 228ms | 16 条规则命中 59 项原始发现（228ms） |
| 聚合去重 | 成功 | 5ms | 59 条异常（抑制 9 条）聚成 1 个根因簇 |
| 稳定性评分 | 成功 | 0ms | 规则算分 20.4（E 严重） |
| AI 智能分析 | 降级 | 4ms | 降级运行（未配置 llm_api_key，无法调用大模型）；1 条根因结论，token 0，耗时 0ms |
| 报告输出 | 成功 | 12ms | Markdown 报告 11716 字符，已写入 run-20260925-100000-S1.md |
