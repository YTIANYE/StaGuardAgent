...
**对比对象**：`run-20260925-100000-S2`（2026-09-27T01:54+08:00）
...
| 阶段 | 状态 | 耗时 | 说明 |
| --- | --- | ---: | --- |
| 数据采集 | 成功 | 667ms | file 通道采集 215676 个点 / 150 条时序（663ms） |
| 标准化清洗 | 成功 | 2650ms | 216000 个有效点 / 150 条时序；缺失率 0.00%，置信度 高 |
| 结构化存储 | 成功 | 231ms | 数据集 S3 就绪（216000 个指标点） |
| 动态基线 | 成功 | 1230ms | 计算 150 条基线，动态基线覆盖率 100%（归档 603894 点） |
| 规则巡检 | 成功 | 167ms | 16 条规则命中 45 项原始发现（167ms） |
...
| 报告输出 | 成功 | 6ms | Markdown 报告 12467 字符，已写入 run-20260925-100000-S3.md |
