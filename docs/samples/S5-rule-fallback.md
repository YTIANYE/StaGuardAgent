...
**对比对象**：`run-20260925-100000-S4`（2026-09-27T01:54+08:00）
...
| 标准化清洗 | 成功 | 2679ms | 216000 个有效点 / 150 条时序；缺失率 0.00%，置信度 高 |
| 结构化存储 | 成功 | 227ms | 数据集 S5 就绪（216000 个指标点） |
| 动态基线 | 成功 | 1236ms | 计算 150 条基线，动态基线覆盖率 100%（归档 603894 点） |
| 规则巡检 | 成功 | 159ms | 16 条规则命中 16 项原始发现（159ms） |
